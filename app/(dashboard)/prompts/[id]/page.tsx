        ...
        return (
            <div className="h-full flex flex-col">
                ...
                <div className="flex-1 p-8 flex flex-col items-center justify-center">
                    <p className="text-slate-400 mb-4">Bu prompt silinmiş veya erişim izniniz yok.</p>
            ...
            <div className="flex-1 p-4 md:p-8 overflow-y-auto w-full max-w-5xl mx-auto space-y-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-1">
                        <Link href="/prompts" className="inline-flex items-center gap-1 text-sm font-medium text-slate-400 hover:text-indigo-400 mb-2 transition-colors">
                                ...
                                <>
                                    ...
                                    <div className="flex gap-1.5">
                                        {currentPrompt.tags.map((t: string) => (
                                            <span key={t} className="bg-slate-800 text-slate-300 px-2 py-0.5 rounded-md text-[11px] font-medium border border-slate-700">
                                                {t}
                                            </span>
                                        ))}
                                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <Link
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-sm font-medium text-slate-200 transition-all active:scale-95"
                        >
                            <GitMerge size={16} />
                            Yeni Versiyon
                        </Link>
                        <Link
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-lg text-sm font-medium text-white transition-all active:scale-95"
                        >
                            <FilePlus2 size={16} />
                            Göreve Ekle
                        </Link>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-sm flex flex-col">
                    <div className="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                        <h3 className="font-semibold text-slate-200">İçerik</h3>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden divide-y divide-slate-800/50">
                            {versionChain.map((v) => {
                                ...
                                return (
                                    <div key={v.id} className={`flex items-start justify-between p-4 md:p-5 transition-colors ${isActive ? 'bg-indigo-500/5' : 'hover:bg-slate-800/30'}`}>
                                        <div className="flex-1 min-w-0 pr-4">
                                            <div className="flex items-center gap-3 mb-1">
                                                <span className={`px-2 py-0.5 rounded text-xs font-semibold shrink-0 border ${isActive ? 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>
                                                    v{v.version}
                                                </span>
                                                {isActive && (
                                            ...
                                            <p className="text-sm font-mono text-slate-400 truncate mt-2">
                                                {v.content}
                                            </p>
                                        </div>
                                        {!isActive && (
                                            <div className="shrink-0 mt-2 md:mt-0 items-center justify-center flex">
                                                <Link
                                                    href={`/prompts/${v.id}`}
                                                    className="inline-flex px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-md text-xs font-medium border border-slate-700 transition-colors"
                                                >
                                                    Görüntüle
                                                </Link>
                                            </div>
